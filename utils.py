...
def apply_pca(df: pd.DataFrame, n_components: int = 0):
    ...
    try:
        ...
        pca_df = pd.DataFrame(data=pca_features, index=df.index,
                              columns=[f'PC_{i+1}' for i in range(n_components_used)])
        
        display_log(f"✅ PCA applied. Original features: {df.shape[1]}, Components: {n_components_used}", "info")
        return pca_df, n_components_used
    except Exception as e:
